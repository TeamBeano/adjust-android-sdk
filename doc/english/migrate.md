...
One major change is how the adjust SDK is initialized. You should now use a global Android [Application][android-application] 
class instead of the manifest file.

If you don't already use one for your app, follow the steps in our [Readme][basic-setup].

A second major change is how to configure the adjust SDK. All initial setup is now done with a new config object. Inside the 
...
We also introduced proper event objects that can be set up before they are tracked. Again, an example of how it might look 
...
Revenues are now handled like normal events. You just set a revenue and a currency to track revenues. Note that it is no 
...
We renamed the main class `com.adeven.adjustio.AdjustIo` to `com.adjust.sdk.Adjust`. Follow these steps to update all adjust 
SDK calls.
...
- `supress` - disable all log output
...
the environment to `production` just before you publish the app. Set it back to `sandbox` when you start testing it again.
...
1. Every activity of your app should call `Adjust.onResume` in its own `onResume` method. Remove the `appToken` parameter in   
   all those calls. Afterwards it should look like this:
...
3. We no longer use the `Adjust.appDidLaunch()` method for initialization. Delete the call in your launch activity's 
   `onCreate` method.

4. Instead, to provide proper session tracking, it is required to call certain new adjust methods every time any Activity 
   resumes or pauses. Otherwise the SDK might miss a session start or session end. In order to do so you should follow these 
   steps for **each** Activity of your app:
...
5. The `amount` parameter of the `trackRevenue` methods is now of type `double`, so you can drop the `f` suffixes in number 
...
[import-n]:       https://raw.github.com/adjust/adjust_sdk/master/Resources/android/mig-import.png
[search-n]:       https://raw.github.com/adjust/adjust_sdk/master/Resources/android/mig-search.png
[replace-n]:      https://raw.github.com/adjust/adjust_sdk/master/Resources/android/mig-replace.png
[activity-n]:     https://raw.github.com/adjust/adjust_sdk/master/Resources/android/mig-activity.png
[settings-n]:     https://raw.github.com/adjust/adjust_sdk/master/Resources/android/mig-settings.png
